...
import { checkPrivacyPolicyStatus } from '../../controllers/PrivacyPolicyController';
import { checkTermsOfServiceStatus } from '../../controllers/TermsOfServiceController';
...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      // Check privacy policy status (non-blocking)
      checkPrivacyPolicyStatus().catch((error) => {
        // Silently fail - don't block user access if check fails
        console.log('Privacy policy status check failed:', error);
      }).then((status) => {
        if (status && !status.hasAcceptedLatestVersion) {
          // Show a non-blocking notification with custom JSX
          toast((t) => (
      ...
      // Check Terms of Service status (non-blocking)
      checkTermsOfServiceStatus().catch((error) => {
        // Silently fail - don't block user access if check fails
        console.log('Terms of Service status check failed:', error);
      }).then((status) => {
        if (status && !status.hasAcceptedLatestVersion) {
          // Show a non-blocking notification with custom JSX
          toast((t) => (
            <div className="flex items-center gap-3">
              <span>📜 Please review and accept our updated Terms of Service</span>
              <button
                onClick={() => {
