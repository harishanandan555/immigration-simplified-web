...
import { CaseStatus, CaseHistoryItem } from '../../types';
...
const CaseStatusPage: React.FC = () => {
  ...
  const handleCaseLookup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiRequest(`/cases/${caseNumber}`, 'GET');
      ...
      if (response.success && response.data) {
        ...
        setError(response.error || 'Failed to retrieve case status. Please check the case number and try again.');
        setCaseStatus(null);
      }
    } catch (error) {
