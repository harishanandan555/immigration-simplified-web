...
const CaseDetailsPage = () => {
    ...
    const fetchCaseData = async () => {
      try {
        ...
        const transformedCase: WorkflowCase = {
          ...
          assignedForms:
            workflowCase.assignedForms || workflow.selectedForms || [],
          formCaseIds: formCaseIds,
          client: {
            _id: workflowClient.id || workflowClient._id || "",
            name:
              workflowClient.name ||
              `${workflowClient.firstName || ""} ${
                workflowClient.lastName || ""
              }`.trim(),
        ...
        try {
          ...
          const caseTasks = allTasks.filter((task: Task) => {
            ...
            return (
              taskCaseId === primaryCaseNumber ||
              taskCaseId === workflow._id ||
              taskCaseId?.includes(primaryCaseNumber) ||
              (taskCaseId &&
                Object.values(formCaseIds).some((caseNum) =>
        ...
        await fetchGeneratedForms(workflow);
      } catch (error: any) {
  ...
  const fetchGeneratedForms = async (workflow: any) => {
    try {
      ...
      // The API now includes pdfData (base64-encoded) directly in the response
      ...
      if (
        ...
        // Process each filled PDF - pdfData is already included as base64
        for (const filledPdf of filledPdfs) {
          try {
            // Check if pdfData is available
            if (!filledPdf.pdfData) {
              ...
              try {
                ...
                if (previewResponse.data && previewResponse.data.blob) {
                  ...
                  forms.push({
                    ...
                    templateId:
                      filledPdf.templateId || metadata?.templateId || "",
                    blob: blob,
                    downloadUrl: URL.createObjectURL(blob),
                    fileName:
                      filledPdf.filename ||
                      metadata?.filename ||
                      `${filledPdf.formNumber}_${Date.now()}.pdf`,
                    pdfId: pdfId || filledPdf._id,
                    status: "success",
                    filledPercentage: metadata?.validationDetails?.filledFields
                      ? (metadata.validationDetails.filledFields /
                          metadata.validationDetails.totalFields) *
                        100
                      : undefined,
                    metadata: {
                      ...metadata,
                      ...
                      contentType:
                        filledPdf.contentType || metadata?.contentType,
                ...
                // Add form with error status if both pdfData and API call fail
                forms.push({
                  formName: filledPdf.formNumber,
              ...
              forms.push({
                ...
                pdfId: filledPdf._id,
                status: "success",
                filledPercentage: undefined, // Not available in this response
                metadata: {
            ...
            console.error(
              `❌ Error processing PDF for ${filledPdf.formNumber}:`,
              formError
            );
        ...
        console.warn(
          "⚠️ No filled PDFs found or API returned unsuccessful response"
        );
      }
  ...
  const handleSaveEditedPdf = async (formName: string, editedPdfBlob: Blob) => {
    try {
      ...
      if (!form.pdfId) {
        toast.error(
          `Cannot save edited PDF: Missing pdfId for form ${formName}`
        );
        return;
      }
      ...
      if (saveResponse.data.success) {
        ...
        const updatedForms = generatedForms.map((f) =>
          f.formName === formName
            ? {
                ...
                blob: editedPdfBlob,
                downloadUrl:
                  saveResponse.data.data?.downloadUrl ||
                  URL.createObjectURL(editedPdfBlob),
        ...
        throw new Error(
          saveResponse.data.message || "Failed to save PDF to database"
        );
      }
    } catch (error: any) {
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow">
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              {caseData.formCaseIds &&
                Object.keys(caseData.formCaseIds).length > 0 && (
                  <div className="col-span-2">
                          ...
                          <div>
                            ...
                            {form.metadata && (
                              <p className="text-sm text-gray-500">
                                ...
                                {form.metadata.fileSize
                                  ? `${(form.metadata.fileSize / 1024).toFixed(
                                      2
                                    )} KB`
                                  : "N/A"}
                              </p>
